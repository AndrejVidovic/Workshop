...
const Workshops=()=>{
    ...
    useEffect(() => { 
        const getData = async () => {
            const res = await fetch(`http://localhost:3000/workshops?&_limit=${pageLimit*currentPage}&_sort=date&_order=desc`,fetchOptions);      
        ...
        {   itemsToFilter.length==0?<ThreeDots color="#0097CC" height={80} width={80} wrapperStyle={{marginTop:"120px",justifyContent:"center"}} />:
            <div className="workshops-container">
                ...
                <div >
                    ...
                    <div className='workshops-body-container'>
                        {
                            window.innerWidth<600?getPaginatedWorkshops().map(workshop=><WorkshopCardSmall workshop={workshop} key={workshop.id}/>):getPaginatedWorkshops().map(workshop=><WorkshopCard workshop={workshop} key={workshop.id}/>)
                        }
                    </div>
                    <div className='load-button-container'>
                        <button onClick={HandleClick} className="load-more">Učitaj više</button>
                    </div>
                </div>
            </div>
        }
